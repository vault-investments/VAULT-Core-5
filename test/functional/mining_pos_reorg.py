...
from test_framework.test_framework import VaultTestFramework
...
class ReorgStakeTest(VaultTestFramework):
    ...
    def check_money_supply(self, expected_vault):
        # verify that nodes have the expected VAULT supply
        vault_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(vault_supply, [DecimalAmt(expected_vault)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that VAULT supply was properly updated after the reorgs
        self.log.info("Check VAULT supply...")
