...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/VAULTParams"
    else
        PARAMS_DIR="$HOME/.vault-params"
    fi
fi
...
function main() {
    ...
    cat <<EOF
VAULT - install-params.sh

This script will install the VAULT zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common VAULT zkSNARK parameters. Note that it is
