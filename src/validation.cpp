...
#include "zvault/zerocoin.h"
#include "zvault/zvaultmodule.h"
...
#if defined(NDEBUG)
#error "VAULT cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uvault) are considered zero fee (for relaying, mining and transaction creation)
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight <= 250001 && nHeight > 245000) {
        nSubsidy = 0.71 * COIN;
    }
...
{
    util::ThreadRename("vault-scriptch");
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZVAULTModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)){
                        return false;
                    }
...
bool CheckWork(const CBlock& block, const CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Vault Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nVaultBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nVaultBadBlockBits)) {
            // accept VAULT block minted with incorrect proof of work threshold
            return true;
        }
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> vaultInputs;
        std::vector<CTxIn> zVAULTInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsZerocoinSpend()){
                zVAULTInputs.push_back(stakeIn);
            }else{
                vaultInputs.push_back(stakeIn);
            }
        }
        const bool hasVAULTInputs = !vaultInputs.empty();
        const bool hasZVAULTInputs = !zVAULTInputs.empty();
        ...
        for (const auto& txIn : block.vtx) {
            ...
            for (const CTxIn& in: tx.vin) {
                if(consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC)) {
                    ...
                    if (isPrivZerocoinSpend || isPublicSpend) {
                        ...
                        if (isPublicSpend) {
                            libzerocoin::ZerocoinParams* params = consensus.Zerocoin_Params(false);
                            PublicCoinSpend publicSpend(params);
                            if (!ZVAULTModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)){
                                return false;
                            }
                            spend = publicSpend;
                        } else {
                ...
                if(hasVAULTInputs) {
                    ...
                    for (const CTxIn& vaultIn : vaultInputs)
                        if(vaultIn.prevout == in.prevout)
        ...
        if (isBlockFromFork) {
            ...
            // Now that this loop if completed. Check if we have zVAULT inputs.
            if(hasZVAULTInputs) {
                for (const CTxIn& zVaultInput : zVAULTInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zVaultInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex->nHeight, UINT256_ZERO))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zvault");
...
int ActiveProtocol()
{
    // SPORK_15 is used for 70922 (v5.2.0)
...
{
    return CScript() << ParseHex("04071002a65c918e29d9ca5986e1a877e7b2e6d6b7ae51af0b23781693f195d1871a0e9167a940101c4e1c47b65854b4fca3696f50f8a7913d8102a33da7dd162d") << OP_CHECKSIG;
}
