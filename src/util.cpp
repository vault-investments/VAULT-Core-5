...
const char * const VAULT_CONF_FILENAME = "vault.conf";
const char * const VAULT_PID_FILENAME = "vault.pid";
const char * const VAULT_MASTERNODE_CONF_FILENAME = "masternode.conf";
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\VAULT
// Windows >= Vista: C:\Users\Username\AppData\Roaming\VAULT
// Mac: ~/Library/Application Support/VAULT
// Unix: ~/.vault
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "VAULT";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\VAULTParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\VAULTParams
    // Mac: ~/Library/Application Support/VAULTParams
    // Unix: ~/.vault-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "VAULTParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "VAULTParams";
#else
    // Unix
    return pathRet / ".vault-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/vault/sapling-spend.params";
        sapling_output = "/usr/share/vault/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/vault/sapling-spend.params";
            sapling_output = "/usr/local/share/vault/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-conf", VAULT_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", VAULT_MASTERNODE_CONF_FILENAME));
...
void ArgsManager::ReadConfigFile()
{
    ...
    if (!streamConfig.good()) {
        // Create empty vault.conf if it does not exist
...

    {
        ...
        for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
            // Don't overwrite existing settings so command line settings override vault.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", VAULT_PID_FILENAME));
