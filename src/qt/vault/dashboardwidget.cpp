...
#include "qt/vault/dashboardwidget.h"
#include "qt/vault/forms/ui_dashboardwidget.h"
#include "qt/vault/sendconfirmdialog.h"
#include "qt/vault/txrow.h"
...
DashboardWidget::DashboardWidget(VAULTGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareVault, "square-chart-vault");
    //setCssProperty(ui->labelSquarezVault, "square-chart-zvault");
    setCssProperty(ui->labelVault, "text-chart-vault");
    //setCssProperty(ui->labelZvault, "text-chart-zvault");
    ...
    setCssProperty(ui->labelAmountVault, "text-stake-vault-disable");
    //setCssProperty(ui->labelAmountZvault, "text-stake-zvault-disable");
    ui->labelAmountMNRewards->setText("0 VAULT");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &VAULTGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("VAULT")
    updateDisplayUnit();
}
...
const QMap<int, QMap<QString, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        //bool isVault = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZVAULT;
        bool isVault = modelIndex.data(TransactionTableModel::TypeRole).toInt() == TransactionRecord::StakeMint;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair VAULT, zVAULT
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            //vault = (pair.first != 0) ? pair.first / 100000000 : 0;
            //zvault = (pair.second != 0) ? pair.second / 100000000 : 0;
            //chartData->totalVault += pair.first;
            //chartData->totalZvault += pair.second;
            QMap<QString, qint64> pair = chartData->amountsByCache[num];
            vault = (pair["vault"] != 0) ? pair["vault"] / 100000000 : 0;
            //zvault = (pair["zvault"] != 0) ? pair["zvault"] / 100000000 : 0;
            mnrewards = (pair["mn"] != 0) ? pair["mn"] / 100000000 : 0;
            chartData->totalVault += pair["vault"];
            //chartData->totalZvault += pair["totalZvault"];
            chartData->totalMNRewards += pair["mn"];
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesVault.append(vault);    
        chartData->valuesMNRewards.append(mnrewards); 
        //chartData->valueszVault.append(zvault);

        int max = std::max(vault, mnrewards);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesVault);
    //set1->append(chartData->valueszVault);
    set1->append(chartData->valuesMNRewards);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    //if (chartData->totalVault > 0 || chartData->totalZvault > 0) {
    if (chartData->totalVault > 0 || chartData->totalMNRewards > 0) {
        setCssProperty(ui->labelAmountVault, "text-stake-vault");
        //setCssProperty(ui->labelAmountZvault, "text-stake-zvault");
    ...
    //forceUpdateStyle({ui->labelAmountVault, ui->labelAmountZvault});
    forceUpdateStyle({ui->labelAmountVault, ui->labelAmountMNRewards});
    ui->labelAmountVault->setText(GUIUtil::formatBalance(chartData->totalVault, nDisplayUnit));
    //ui->labelAmountZvault->setText(GUIUtil::formatBalance(chartData->totalZvault, nDisplayUnit, true));
