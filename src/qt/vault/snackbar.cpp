...
#include "qt/vault/forms/ui_snackbar.h"
#include "qt/vault/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(VAULTGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &VAULTGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
