...
#include "qt/vault/send.h"
#include "qt/vault/forms/ui_send.h"
#include "qt/vault/addnewcontactdialog.h"
#include "qt/vault/qtutils.h"
#include "qt/vault/sendchangeaddressdialog.h"
#include "qt/vault/optionbutton.h"
#include "qt/vault/sendconfirmdialog.h"
#include "qt/vault/guitransactionsutils.h"
#include "qt/vault/loadingdialog.h"
...
SendWidget::SendWidget(VAULTGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-vault");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onVAULTSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onVAULTSelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent VAULTs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res.result) {
                inform(tr("Error generating address to shield VAULTs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent VAULTs to shield."));
    }
}
...
void SendWidget::onVAULTSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto vaultAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(vaultAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(vaultAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(vaultAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
