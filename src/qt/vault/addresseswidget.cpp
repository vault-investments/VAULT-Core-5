...
#include "qt/vault/forms/ui_addresseswidget.h"
#include "qt/vault/addresslabelrow.h"
#include "qt/vault/addnewaddressdialog.h"
#include "qt/vault/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(VAULTGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto vaultAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(vaultAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(vaultAdd);
        if (walletModel->updateAddressBookLabels(vaultAdd, label.toUtf8().constData(),
