...
#include "qt/vault/settings/settingsfaqwidget.h"
#include "qt/vault/settings/forms/ui_settingsfaqwidget.h"
#include <QScrollBar>
#include <QMetaObject>
#include "qt/vault/qtutils.h"

SettingsFaqWidget::SettingsFaqWidget(VAULTGUI *parent) :
    QDialog(parent),
    ui(new Ui::SettingsFaqWidget)
{
    ...
    setCssProperty({
           ...
           ui->labelNumber_UnspendableVAULT,
    ...
    setCssProperty({
              ...
              ui->labelSubtitle_UnspendableVAULT,
    ...
    setCssProperty({
              ...
              ui->labelContent_UnspendableVAULT,
    ...
    setCssProperty({
              ui->pushButton_Intro,
              ui->pushButton_UnspendableVAULT,
              ui->pushButton_Stake,
    ...
    QString introContent = formatFAQContent(
        formatFAQParagraph(
            tr("VAULT is a form of digital online money using blockchain technology "
               "that can be easily transferred globally, instantly, and with near "
               "zero fees. VAULT incorporates market leading security & "
               "privacy and is also the first PoS (Proof of Stake) Cryptocurrency "
               "to implement Sapling(SHIELD), a zk-SNARKs based privacy protocol.")) +
        formatFAQParagraph(
            tr("VAULT utilizes a Proof of Stake (PoS) consensus system algorithm, "
               "allowing all owners of VAULT to participate in earning block rewards "
               "while securing the network with full node wallets, as well as to "
               "run Masternodes to create and vote on proposals.")));
    ui->labelContent_Intro->setText(introContent);

    QString unspendableVAULTContent = formatFAQContent(
        formatFAQParagraph(
            tr("Newly received VAULT requires 6 confirmations on the network "
               "to become eligible for spending which can take ~6 minutes.")) +
        formatFAQParagraph(
            tr("Your VAULT wallet also needs to be completely synchronized "
               "to see and spend balances on the network.")));
    ui->labelContent_UnspendableVAULT->setText(unspendableVAULTContent);
    ...
    QString stakeContent = formatFAQContent(
        formatFAQOrderedList(
            ...
            formatFAQListItem(tr("You must have a balance of VAULT with a minimum of 600 confirmations.")) +
        ...
        formatFAQParagraph(
            tr("You can see the status of staking in the wallet by mousing over the "
               ...
               "will confirm that staking is active.")
                .arg("vault-core", "<span style=\"font-style:italic\">getstakingstatus</span>")));
    ...
    QString supportContent = formatFAQContent(
        formatFAQParagraph(
            tr("We have support channels in most of our official chat groups, for example %1")
                .arg("<a style='color: #b088ff' href='https://discord.VAULT.org'>" + tr("#support in our Discord") + "</a>.")));
    ui->labelContent_Support->setText(supportContent);

    QString masternodeContent = formatFAQContent(
        formatFAQParagraph(
            tr("A masternode is a computer running a full node VAULT core wallet with a "
               "requirement of 10,000 VAULT secured collateral to provide extra services "
        ...
        formatFAQParagraph(
            tr("Masternode Perks:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Participate in VAULT Governance")) +
                formatFAQListItem(tr("Earn Masternode Rewards")) +
                formatFAQListItem(tr("Commodity option for future sale")) +
                formatFAQListItem(tr("Help secure the VAULT network")))) +
        formatFAQParagraph(
            tr("Requirements:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("10,000 VAULT per single Masternode instance")) +
    ...
    QString mNControllerContent = formatFAQContent(
        formatFAQParagraph(
            tr("A Masternode Controller wallet is where the 10,000 VAULT collateral "
    ...
    connect(ui->pushButton_UnspendableVAULT, &QPushButton::clicked, [this](){onFaqClicked(ui->widget_UnspendableVAULT);});
    ...
    if (parent)
        connect(parent, &VAULTGUI::windowResizeEvent, this, &SettingsFaqWidget::windowResizeEvent);
}
