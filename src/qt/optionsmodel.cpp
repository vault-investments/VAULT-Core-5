...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::VAULT);
    ...
    if (settings.contains("nAnonymizeVaultAmount") || reset)
        gArgs.SoftSetArg("-anonymizevaultamount", settings.value("nAnonymizeVaultAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in vault.cpp during shotdown to also remove the window positions
