...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select VAULT Outputs to Spend" :
            "Select Shielded VAULT to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::VAULT;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show VAULT address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
