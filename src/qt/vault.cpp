...
#include "qt/vault/splash.h"
#include "qt/vault/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("vault-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in vault.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in vault.qrc)
...
/** Class encapsulating VAULT Core startup and shutdown.
...
public:
    ...
    /// Get window identifier of QMainWindow (VAULTGUI)
...
{
    window = new VAULTGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &VAULTGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &VAULTGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(VAULTGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(VAULTGUI::DEFAULT_WALLET);
        ...
        // VAULT: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &VAULTGUI::handlePaymentRequest);
        connect(window, &VAULTGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. VAULT can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(vault_locale);
    Q_INIT_RESOURCE(vault);
    ...
    /// 6. Determine availability of data directory and parse vault.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("VAULT Core"),
    ...
    try {
        ...
        QMessageBox::critical(0, QObject::tr("VAULT Core"), QObject::tr("Error: %1").arg(e.what()));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // vault: links repeatedly have their payment requests routed to this process:
