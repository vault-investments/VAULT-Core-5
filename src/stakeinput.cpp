...
CVaultStake* CVaultStake::NewVaultStake(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend()) {
        error("%s: unable to initialize CVaultStake from zerocoin spend", __func__);
        return nullptr;
    }
    ...
    return new CVaultStake(txPrev->vout[txin.prevout.n],
                         txin.prevout,
                         pindexFrom);
}

bool CVaultStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}

bool CVaultStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(outpointFrom.hash, outpointFrom.n);
    return true;
}

CAmount CVaultStake::GetValue() const
{
    return outputFrom.nValue;
}

bool CVaultStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CVaultStake::GetUniqueness() const
{
    //The unique identifier for a VAULT stake is the outpoint
...
const CBlockIndex* CVaultStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("CVaultStake: uninitialized pindexFrom");
    return pindexFrom;
}

// Verify stake contextual checks
bool CVaultStake::ContextCheck(int nHeight, uint32_t nTime)
{
