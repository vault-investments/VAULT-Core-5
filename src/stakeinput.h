...
#ifndef VAULT_STAKEINPUT_H
#define VAULT_STAKEINPUT_H
...
public:
    ...
    virtual bool IsZVAULT() const = 0;
    virtual CDataStream GetUniqueness() const = 0;
    virtual bool ContextCheck(int nHeight, uint32_t nTime) = 0;
};


class CVaultStake : public CStakeInput
{
private:
...
public:
    CVaultStake(const CTxOut& _from, const COutPoint& _outPointFrom, const CBlockIndex* _pindexFrom) :
            CStakeInput(_pindexFrom), outputFrom(_from), outpointFrom(_outPointFrom) {}

    static CVaultStake* NewVaultStake(const CTxIn& txin);
    ...
    bool IsZVAULT() const override { return false; }
    bool ContextCheck(int nHeight, uint32_t nTime) override;
};


#endif //VAULT_STAKEINPUT_H
