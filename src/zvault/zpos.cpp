...
bool CLegacyZVaultStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZVaultStake from non zc-spend", __func__);
    ...
    *this = CLegacyZVaultStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zvault stake origin", __func__);

    // All good
    return true;
}

CLegacyZVaultStake::CLegacyZVaultStake(const libzerocoin::CoinSpend& spend) : CStakeInput(nullptr)
{
...
const CBlockIndex* CLegacyZVaultStake::GetIndexFrom() const
{
...
CAmount CLegacyZVaultStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZVaultStake::GetUniqueness() const
{
...
bool CLegacyZVaultStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zVAULT stake block: height %d outside range", __func__, nHeight);
