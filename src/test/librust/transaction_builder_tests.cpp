...
BOOST_AUTO_TEST_CASE(TransparentToSapling)
{
    ...
    // 0.5 t-VAULT in, 0.4 shielded-VAULT out, 0.1 t-VAULT fee
...
BOOST_AUTO_TEST_CASE(SaplingToSapling) {
    ...
    // --- 0.4 shielded-VAULT in, 0.299 shielded-VAULT out, 0.1 shielded-VAULT fee, 0.001 shielded-VAULT change (added to fee)
    ...
    // --- Now try with 1 shielded-VAULT in, 0.5 shielded-VAULT out, 0.1 shielded-VAULT fee, 0.4 shielded-VAULT change
...
BOOST_AUTO_TEST_CASE(FailsWithNegativeChange)
{
    ...
    // Generate a 0.5 VAULT note
    auto testNote = GetTestSaplingNote(pa, 59990000);

    // Fail if there is only a Sapling output
    // 0.5 shielded-VAULT out, 0.1 t-VAULT fee
    auto builder = TransactionBuilder(consensusParams, 1);
    ...
    // 0.5 t-VAULT out, 0.1 t-VAULT fee
    ...
    // 0.5 t-VAULT out, 0.1 t-VAULT fee, 0.59999 shielded-VAULT in
    builder.AddSaplingSpend(expsk, testNote.note, testNote.tree.root(), testNote.tree.witness());
    BOOST_CHECK_EQUAL("Change cannot be negative", builder.Build().GetError());
