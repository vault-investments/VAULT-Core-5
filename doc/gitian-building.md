...
*Setup instructions for a gitian build of VAULT Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the VAULT
...
to the VAULT GitHub release page.
...
- [Building VAULT Core](#building-vault-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/vault-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/vault-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/vault-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/vault-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/vault-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/vault-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/vault-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@vault.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/vault-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/vault-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/vault-osx-*-build.assert
```
