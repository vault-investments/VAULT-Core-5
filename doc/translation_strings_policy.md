...
This document provides guidelines for internationalization of the VAULT Core software.
...
This adds `<numerusform>`s to the respective `.ts` file, which can be translated separately depending on the language. In English, this is simply:
    ...
    <message numerus="yes">
        <source>%n active connection(s) to VAULT network</source>
        <translation>
            <numerusform>%n active connection to VAULT network</numerusform>
            <numerusform>%n active connections to VAULT network</numerusform>
        </translation>
    </message>
...
This can be checked by executing `make translate` in the `src` directory, then verifying that `vault_en.ts` remains unchanged.
